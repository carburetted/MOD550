...
class DeliveryWithinArea():
    ...
    def leppard86(self, cons_e = 30, serv_e_10k = 1400):
        ...
        self.df_sorted_e["vehicle_type"] = "Electric"
        return self.df_sorted_e  # returning raw dataframe (with all columns)
    ...
    def leppard87(self, cons_d = 10, serv_d_10k = 3000):
        ...
        self.df_sorted_d["vehicle_type"] = "Diesel"
        return self.df_sorted_d  # returning raw dataframe (with all columns)
    ...
    def save_dataset(self, filename="exercise_1_dataset.csv"):  # concatenation and saving of datasets
        
        df_electric = plot.leppard86()
        df_diesel = plot.leppard87()
        # keeping only necessary columns (for X and Y values):
        df_electric = df_electric[["Cumulative operational costs", "Cumulative amount of buildings"]]
        df_diesel = df_diesel[["Cumulative operational costs", "Cumulative amount of buildings"]]

        self.df_combined = pd.concat([df_electric, df_diesel], ignore_index=True) # appending together the two datasets

        self.df_combined.to_csv(filename, index=False)
...
# import of datasets from others:
url_d = "https://raw.githubusercontent.com/eloraafrin14/MOD550/refs/heads/main/data/combined_dataset.csv"
df_other = pd.read_csv(url_d)
df_other.to_csv("exercise_1_imported_dataset.csv", index=False) # saving
print("Imported dataset saved")

# import of metadata:
url_m = "https://raw.githubusercontent.com/eloraafrin14/MOD550/refs/heads/main/data/combined_metadata.json"
metadata_other = json.loads(requests.get(url_m).text)
with open("exercise_1_imported_metadata.json", "w") as f:
            json.dump(metadata_other, f, indent=4)
...
plt.plot(x, np.sin(x), c = 'y') # the truth of the imported dataset seems to be function y = sin x, where x = [0..2П] / confirmed after looking into metadata file!
plt.scatter(df_other["x"], df_other["y"])
plt.xlabel("X")
plt.xlabel("Y")
